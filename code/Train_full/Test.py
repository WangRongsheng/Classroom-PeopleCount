...
pic = cv2.imread('./Photos/test_'+str(number)+'.jpg', cv2.IMREAD_GRAYSCALE)  # 直接读取灰度图像
a = []
b = []
...
for i in range(0,round(len(a)/2)):
    ...
    # 显示边框
    pic[b[2 * i] - 1:b[2 * i], a[2 * i]:a[2 * i + 1]] = (255, 255, 255)
    pic[b[2 * i + 1]:b[2 * i + 1] + 1, a[2 * i]:a[2 * i + 1]] = (255, 255, 255)
    pic[b[2 * i]:b[2 * i + 1], a[2 * i] - 1:a[2 * i]] = (255, 255, 255)
    pic[b[2 * i]:b[2 * i + 1], a[2 * i + 1]:a[2 * i + 1] + 1] = (255, 255, 255)
